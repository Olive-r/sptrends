...
.summary_tst <- function(object, ...) {
  ...
  if (!is.null(object$fdr)) {
    cat("=== FDR correction (the actual TST result) ===\n")
...
.plot_tst <- function(x, which = c("direction", "significance", "trend",
                                   ...
                                   "pvalue_bar"),
                      method = c("BKY", "BH", "BY"), smooth = TRUE, ...) {
  ...
  if (which == "direction") {
    ...
    reject_dir <- x$fdr[[paste0("reject_", method)]]
    if (is.null(reject_dir)) {
      stop(sprintf(
    ...
    slope_for_direction <- if (is.null(x$theil_sen)) {
      NULL
    } else if (already_smoothed_dir) {
      x$theil_sen
    } else {
      suppressMessages(
    ...
    direction <- direction_map(x$trend, x$fdr, slope = slope_for_direction,
                                    method = method,
                                    verbose = FALSE)
    fdr_direction_plot(direction, main = "TST direction map")
    ...
    reject <- x$fdr[[paste0("reject_", method)]]
    if (is.null(reject)) {
      stop(sprintf(
        paste0("FDR-%s is not available in this workflow_tst object -- ",
               "rerun workflow_tst() with that method included, or set ",
