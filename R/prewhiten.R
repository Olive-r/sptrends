...
.theil_sen_pairs <- function(t, max_pairs = 2000L, seed = 1L) {
  ...
  if (n_pairs_total > max_pairs) {
    # withr::with_seed() sets the seed, runs the given expression, and
    # restores the caller's own random state afterward -- without this
    # package's own code ever touching .Random.seed / .GlobalEnv
    # directly (CRAN policy prohibits packages from doing so
    # themselves, even to save and restore it correctly; withr is the
    # standard, CRAN-approved way to get the same reproducibility
    # guarantee without that direct access).
    keep <- withr::with_seed(seed, sample.int(n_pairs_total, max_pairs))
...
prewhiten_histograms <- function(diagnostics, path = NULL) {
  ...
  if (any(is.finite(dw))) {
    plot_dw()
    if (!is.null(path)) .save_current_plot(paste0(path, "_dw.png"), plot_dw)
  }

  rho_mod <- rho[!is.na(mod) & mod == 1 & is.finite(rho)]
  if (length(rho_mod) > 0) {
    plot_rho <- function() {
      graphics::hist(rho_mod, breaks = 40, col = "orange", border = "white",
           main = "Distribution of rho (prewhitened cells only)",
           xlab = "rho")
    }
    plot_rho()
    if (!is.null(path)) .save_current_plot(paste0(path, "_rho.png"), plot_rho)
  } else {
...
prewhiten_maps <- function(diagnostics, path = NULL) {
  ...
  plot_dw_map <- function() {
    terra::plot(diagnostics$DW_initial, col = pal_seq,
                main = "Initial Durbin-Watson")
  }
  if (any(is.finite(terra::values(diagnostics$DW_initial, mat = FALSE)))) {
    plot_dw_map()
    if (!is.null(path)) {
      .save_current_plot(paste0(path, "_dw_map.png"), plot_dw_map)
    }
  }
  ...
  plot_mod_map()
  if (!is.null(path)) {
    .save_current_plot(paste0(path, "_modified_map.png"), plot_mod_map)
  }
