...
slope_summary <- function(slope, path = NULL) {
  ...
  tab <- data.frame(
    metric = c("valid_cells", "min_slope", "median_slope", "mean_slope",
               "max_slope", "pct_increasing", "pct_decreasing", "pct_flat"),
    value = c(n_valid, round(min(v), 6), round(stats::median(v), 6),
              round(mean(v), 6), round(max(v), 6),
              round(100 * n_pos / n_valid, 2), round(100 * n_neg / n_valid, 2),
              round(100 * n_zero / n_valid, 2))
  )
...
slope_map <- function(slope, path = NULL) {
  ...
  terra::plot(slope, col = grDevices::hcl.colors(50, "Blue-Red 3"),
              range = range_lim, fill_range = TRUE, main = "Theil-Sen slope")
  if (!is.null(path)) .save_current_plot(paste0(path, "_theilsen_map.png"))
  invisible(NULL)
}
...
slope_direction_barplot <- function(slope, probability = FALSE, ...) {
  ...
  graphics::barplot(vals_bar, col = c("forestgreen", "firebrick", "grey85"),
                     main = "Direction of the Theil-Sen slope",
                     ylab = ylab, ...)
  invisible(counts)
}
