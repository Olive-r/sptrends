...
.summary_compare_detections <- function(object, ...) {
  ...
  # FPR and FDR (both are error rates: a LOWER value is better) --
  # picking the "winner" with which.max() for those two would name the
  # method with the *most* false positives/discoveries as the best one.
  lower_is_better <- c("FPR", "FDR")
