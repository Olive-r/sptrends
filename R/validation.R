...
.score_one_run <- function(detections, ground_truth, metrics,
  ...
  domains <- lapply(detections, function(detection) {
    if (is.list(detection) && !inherits(detection, "SpatRaster")) {
      if (is.null(detection$significant)) {
        stop("Detection list has no 'significant' element.")
      }
      detection <- detection$significant
    }
    sig <- to_logical(detection)
    if (length(sig) != length(truth_logical)) {
      stop("Detection and ground truth differ in length.")
    }
    !is.na(sig) & !is.na(truth_logical) & !is.na(shared_mask) & shared_mask
  })
  if (length(domains) > 1L &&
      !all(vapply(domains, identical, logical(1), domains[[1]]))) {
  ...
  rows <- lapply(names(detections), function(nm) {
    ...
    # Nota: no se repiten aqui las comprobaciones de "tiene
    # $significant" ni de "longitud coincide con ground_truth" -- el
    # bucle de 'domains' arriba ya las hace, para CADA elemento de
    # 'detections', antes de que este bucle se ejecute siquiera.
    if (is.list(detection) && !inherits(detection, "SpatRaster")) {
