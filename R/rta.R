...
#'   has no top-level `t` argument of its own -- if your observation
#'   times are irregularly spaced and you need a genuine time axis for
#'   [trend_test()]/[slope_estimator()] rather than the default `1:n`
#'   layer index, you must supply the same correct `t` yourself in
#'   both `cmk_args` and `theil_sen_args`, and keep them consistent by
#'   hand.
...
#'   "Computational considerations" above. See `cmk_args` above for a
#'   known limitation with irregular time spacing if you pass `t` here.
...
#'     output (a `SpatRaster`) -- not the full return value. Not
#'     smoothed by default -- check `theil_sen_smoothed` if you need
...
#'     `SpatRaster`) -- not the full return value, which also includes
