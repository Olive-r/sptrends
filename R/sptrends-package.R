...
#'   draws, deliberately split into three distinct schemes so no two
#'   different questions ever share a colour**: continuous diverging
#'   maps (Theil-Sen slope, the S/Sm trend statistic, rho) use a
#'   genuine ColorBrewer RdBu ramp (`#2166ac` blue through white to
#'   `#b2182b` red, colourblind-safe and domain-neutral -- not
#'   `"Green-Brown"`, which reads naturally as vegetation
#'   greening/browning but would be a strange default for, say, a
#'   temperature or precipitation series; that palette remains
#'   available and worth using explicitly for NDVI-like data
...
#'   barely-distinguishable band near the palette's midpoint.
#'   Categorical direction-of-change results (decrease/increase) use
#'   a separate red/blue pair instead (Tableau's `tab:red`/`tab:blue`,
#'   `"#d62728"`/`"#1f77b4"`) -- visually distinct from the continuous
#'   ramp above, so a categorical result is never mistaken for a
#'   continuous one at a glance. Purely binary significant/
#'   non-significant results (`fdr_significance_maps()`,
#'   `fdr_threshold_plot()`) use this package's own chosen magenta
#'   (`"#bf3688"`, `.sptrends_brand$navy`, internal) for "significant"
#'   against a neutral grey for "not significant" -- deliberately a
#'   third, unrelated colour, so "is this significant" is never
#'   confused with either of the other two, direction-related
#'   schemes above; the one colour a reader learns to recognise as
#'   this package's consistent "yes" signal, wherever it appears.
#'
#' @references
