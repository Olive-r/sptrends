...
#'   \item{`"monthly"`}{One date per calendar month, the 1st.
...
#'   \item{`"semimonthly"`}{Two dates per calendar month, the 1st and
...
#'     ..., 358 -- unlike `"8-day"`/`"16-day"`, the last day
#'     of the year belongs to no period and is skipped straight into
...
#'   `"10-day"`, `"16-day"`, `"8-day"` or `"weekly"` (a period
#'   has no start/centre/end ambiguity for `"annual"` or `"daily"`).
...
read_ordered_stack <- function(
  ...
  if (!is.null(cycle_type) &&
    ...
    stop("'time_anchor' only applies to 'monthly', 'semimonthly', ",
         ...
         "'annual' and 'daily' periods have no start/centre/end ",
