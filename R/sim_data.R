...
sim_trend_stack <- function(nrow = 15, ncol = 15, n_time = 10,
  ...
  if (!is.numeric(nrow) || length(nrow) != 1L || is.na(nrow) ||
      !is.finite(nrow) || nrow < 1 || nrow != floor(nrow)) {
    stop("'nrow' must be one positive integer.")
  }
  if (!is.numeric(ncol) || length(ncol) != 1L || is.na(ncol) ||
      !is.finite(ncol) || ncol < 1 || ncol != floor(ncol)) {
    stop("'ncol' must be one positive integer.")
  }
  if (!is.numeric(n_time) || length(n_time) != 1L || is.na(n_time) ||
      !is.finite(n_time) || n_time < 1 || n_time != floor(n_time)) {
    stop("'n_time' must be one positive integer.")
  }
  if (!is.numeric(trend_strength) || length(trend_strength) != 1L ||
      is.na(trend_strength) || !is.finite(trend_strength)) {
    stop("'trend_strength' must be one finite numeric value.")
  }
  if (!is.numeric(break_magnitude) || length(break_magnitude) != 1L ||
      is.na(break_magnitude) || !is.finite(break_magnitude)) {
    stop("'break_magnitude' must be one finite numeric value.")
  }
  if (!is.logical(constant_block) || length(constant_block) != 1L ||
      is.na(constant_block)) {
    stop("'constant_block' must be one logical value (TRUE or FALSE).")
  }
  ...
  if (!is.numeric(ar1) || length(ar1) != 1L || is.na(ar1) ||
      !is.finite(ar1) || ar1 <= -1 || ar1 >= 1) {
    stop("'ar1' must be one finite numeric value in (-1, 1).")
  }
  if (!is.numeric(noise_sd) || length(noise_sd) != 1L || is.na(noise_sd) ||
      !is.finite(noise_sd) || noise_sd < 0) {
    stop("'noise_sd' must be one non-negative finite numeric value.")
  }
