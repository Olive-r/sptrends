...
#' @noRd
.benchmark_scenario_data <- function(arguments, simulation, evaluation_mask = NULL) {
  reserved <- c(
    "Scenario", "Replicate", "Seed", "Elapsed", "Method",
    "DomainCells", "TrueNulls", "Pi0", "SignalProportion")
  ...
  if (is.null(truth)) {
    ...
    valid <- !is.na(truth)
    if (!is.null(evaluation_mask)) {
      mask <- if (inherits(evaluation_mask, "SpatRaster")) {
        terra::values(evaluation_mask, mat = FALSE)
      } else as.vector(evaluation_mask)
      if (length(mask) != length(truth)) {
        stop("'evaluation_mask' and ground truth differ in length.")
      }
      valid <- valid & !is.na(mask) & mask != 0
    }
    m <- sum(valid)
...
.benchmark_score <- function(outputs, simulation, stage, evaluator, metrics,
  ...
  if (stage == "prewhitening") {
    return(.benchmark_prewhitening(outputs, simulation, evaluation_mask))
  }
  ...
  truth <- if (inherits(simulation$true_slope, "SpatRaster")) {
    terra::values(simulation$true_slope, mat = FALSE)
  } else as.vector(simulation$true_slope)
  estimates <- lapply(outputs, function(x) {
    if (inherits(x, "SpatRaster")) terra::values(x, mat = FALSE) else as.vector(x)
  })
  domain <- .benchmark_common_domain(
    lapply(estimates, is.finite), truth, evaluation_mask)
  rows <- lapply(names(outputs), function(method_name) {
    estimate <- estimates[[method_name]]
    # Nota: no se repite aqui la comprobacion de longitud -- ya la
    # hace .benchmark_common_domain() arriba, para CADA elemento de
    # 'estimates', antes de que este bucle se ejecute siquiera.
    ok <- domain
...
#' @noRd
.benchmark_prewhitening <- function(outputs, simulation, evaluation_mask = NULL) {
  if (is.null(simulation$true_slope)) {
    stop("stage = 'prewhitening' requires the simulator's output to ",
         "include 'true_slope' (missing here -- check your 'simulator' ",
         "function).")
  }
  true_slope <- if (inherits(simulation$true_slope, "SpatRaster")) {
    terra::values(simulation$true_slope, mat = FALSE)
  } else as.vector(simulation$true_slope)
  validity <- lapply(outputs, function(output) {
    transformed <- if (is.list(output) && !is.null(output$series)) {
      output$series
    } else output
    if (!inherits(transformed, "SpatRaster")) {
      stop("Prewhitening output must contain a SpatRaster series.")
    }
    values <- terra::values(transformed, mat = TRUE)
    if (nrow(values) != length(true_slope)) {
      stop("Prewhitening output and truth differ in cell count.")
    }
    rowSums(is.finite(values)) == ncol(values)
  })
  domain <- .benchmark_common_domain(validity, true_slope, evaluation_mask)
  rows <- lapply(names(outputs), function(method_name) {
    ...
    # Nota: no se repiten aqui las comprobaciones de "es SpatRaster" ni
    # de "cuenta de celdas" -- el bucle de 'validity' arriba ya las
    # hace, para CADA elemento de 'outputs', antes de que este bucle
    # se ejecute siquiera. Solo se comprueba aqui lo que aun no se
    # habia comprobado: el numero minimo de pasos temporales.
    values <- terra::values(transformed, mat = TRUE)
    if (ncol(values) < 3L) {
      stop(sprintf(
    ...
    if (is.list(output) && inherits(output$diagnostics, "SpatRaster") &&
      ...
      modified_fraction <- mean(modified[domain] != 0, na.rm = TRUE)
    }
    data.frame(
      Method = method_name,
      ResidualACF1 = .benchmark_mean(lag1[domain]),
      AbsResidualACF1 = .benchmark_mean(abs(lag1[domain])),
      SlopeBias = .benchmark_mean(slope_error[domain]),
      SlopeMAE = .benchmark_mean(abs(slope_error[domain])),
      SlopeRMSE = sqrt(.benchmark_mean(slope_error[domain]^2)),
...
#' @noRd
.benchmark_common_domain <- function(validity, truth, evaluation_mask) {
  base <- is.finite(truth)
  if (!is.null(evaluation_mask)) {
    mask <- if (inherits(evaluation_mask, "SpatRaster")) {
      terra::values(evaluation_mask, mat = FALSE)
    } else as.vector(evaluation_mask)
    ...
    base <- base & !is.na(mask) & mask != 0
  }
  domains <- lapply(validity, function(ok) {
    if (length(ok) != length(truth)) {
      stop("Method output and ground truth differ in length.")
    }
    ok & base
  })
  if (!all(vapply(domains, identical, logical(1), domains[[1]]))) {
    stop("Methods have different valid evaluation domains. Supply one ",
         "common 'evaluation_mask' excluding cells unavailable to any method.")
  }
  if (!any(domains[[1]])) stop("No valid cells in the evaluation domain.")
  domains[[1]]
}
