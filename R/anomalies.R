...
#'   calendar with the same positions in every year.
...
#'     for each position in the cycle), ordered from position 1 to
#'     `cycle`, regardless of `start_position`.}
...
#'     in the cycle), in the same order as `climatology`.}
...
compute_anomalies <- function(x, cycle = 12, cycle_type = NULL,
  ...
  # does not use the real dates in terra::time(x)), silently
  ...
  # would otherwise occupy. Numeric cycles also require a consistent
  # set of positions in every cycle.
  ...
  # tapp returns groups in order of first appearance. Restore cycle
  # order before using cycle positions as layer indices.
  cycle_order <- order(unique(position))
  clim <- clim[[cycle_order]]
