...
#' @param window_size Odd integer greater than or equal to `3`. Width and
#'   height, in raster cells, of the square neighbourhood used only by
#'   `method = "CMK"` (see `connectivity` below for its shape). The default
#'   `3L` preserves the CMK region described
...
#'   neighbours, no diagonals). Only used when `method = "CMK"`. `"queen"`
#'   preserves the CMK region described by Neeti and Eastman (2011);
#'   `"rook"` is a narrower neighbourhood, useful when diagonal-only
#'   adjacency is not a meaningful spatial relationship for the process
#'   being tested, or to check how sensitive a result is to the choice of
#'   neighbourhood shape. A non-default value with another `method` is
#'   rejected.
...
#'   accepted only when matrix dimensions, raster geometry, the requested
...
trend_test <- function(x, method = c("CMK", "MK", "OLS", "MMK"), ties = FALSE,
  ...
  if (!neighbourhood && connectivity != "queen") {
    stop("'connectivity' is only applicable when method = \"CMK\".")
  }
  ...
  if (is.null(precomputed_neighbourhood)) {
    precomputed_neighbourhood <- prepare_cmk_neighbourhood(
      x, ok, connectivity = connectivity, window_size = window_size
    )
  }
  ...
  if (isTRUE(continuity)) {
    ...
    #
    # NOTE (flagged by external audit, not yet resolved): with
    # Sm == 0 exactly, D <- (Sm > 0) * 1 gives D = 0, so Zm = 1 /
    # sqrt(VarSm) -- a non-zero Z despite there being no trend
    # evidence in that cell. The mathematically "correct" continuity
    # correction would use a genuine three-way sign (-1/0/+1) instead,
    # giving Zm = 0 at Sm == 0. This has deliberately NOT been changed
    # here: this whole branch exists specifically to match ConMK's own
    # external output (see the frozen-reference test in
    # test-cmk-extra.R), and it is not known whether ConMK's own
    # implementation has the same Sm == 0 behaviour -- changing this
    # without checking ConMK's own source risks breaking agreement
    # with the very tool this branch exists to match. Needs checking
    # against ConMK's own source before deciding whether to correct
    # this or document it as a known, intentional quirk of ConMK
    # compatibility mode.
    D <- (Sm > 0) * 1
...
trend_histograms <- function(trend, path = NULL) {
  ...
  plot_stat()
  if (!is.null(path)) {
    .save_current_plot(paste0(path, "_stat.png"), plot_stat)
  }
  ...
  plot_p()
  if (!is.null(path)) {
    .save_current_plot(paste0(path, "_p.png"), plot_p)
  }
...
trend_maps <- function(trend, alpha = 0.05,
  ...
  if ("statistic" %in% panels) {
    ...
    plot_stat_map <- function() {
      terra::plot(trend[[Sname]], col = pal_div, range = range_lim_S,
                  fill_range = TRUE,
                  main = paste(Sname, "-- trend statistic"))
    }
    plot_stat_map()
    if (!is.null(path)) {
      .save_current_plot(paste0(path, "_Sm_map.png"), plot_stat_map)
    }
  }
  ...
  if ("pvalue" %in% panels) {
    ...
    plot_p_map()
    if (!is.null(path)) {
      .save_current_plot(paste0(path, "_p_map.png"), plot_p_map)
    }
  }
  ...
  if ("significance" %in% panels) {
    plot_sig_map <- function() {
      .safe_categorical_plot(sig, values = c(0, 1),
                              colours = c("#D9D9D9", .sptrends_brand$navy),
                              labels = c("Not significant", "Significant"),
                              main = sprintf(
                                "Significant (alpha=%.2f, uncorrected)",
                                alpha),
                              reverse = TRUE)
    }
    plot_sig_map()
    if (!is.null(path)) {
      .save_current_plot(paste0(path, "_significant_map.png"), plot_sig_map)
    }
  }
  ...
  if ("direction" %in% panels) {
    ...
    plot_dir_map()
    if (!is.null(path)) {
      .save_current_plot(paste0(path, "_direction_map.png"), plot_dir_map)
    }
  }
