...
#'     non-CMK methods.}
#'   \item{method}{The requested trend-test method.}
#'   \item{connectivity}{For CMK, the selected queen or rook connectivity.}
...
trend_test <- function(x, method = c("CMK", "MK", "OLS", "MMK"), ties = FALSE,
  ...
  if (!is.null(shared_cluster)) {
    n_cores <- .validate_positive_integer(length(shared_cluster), "shared_cluster size")
  }
  ...
  if (verbose) {
    message(sprintf(
      paste0("[2/3] %s %dx%d neighbourhood, Sm (Eq. 7) and adjusted ",
             "variance (Eqs. 11-13)..."),
      if (connectivity == "queen") "Queen" else "Rook",
  ...
  if (isTRUE(continuity)) {
    ...
    #
    # Compatibility convention: at Sm == 0 this retains the +1 branch,
    # as documented in the continuity argument and the frozen ConMK data.
    D <- (Sm > 0) * 1
