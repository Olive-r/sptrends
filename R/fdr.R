...
#' used internally by [fdr_correction()]. Exported as a standalone
#' convenience for callers who only need a plain vector of p-values
#' corrected, without a raster or the rest of [fdr_correction()]'s own
#' output structure; call `fdr_correction(p, method = "BH")` instead for
#' the full result (rasters, when `p` is one, plus the other methods).
#'
#' @section Typical use:
...
#'
#' @param p Numeric vector of raw p-values in `[0, 1]` (may contain `NA`).
#' @param q Numeric. Target FDR level.
...
#' used internally by [fdr_correction()]. Exported as a standalone
#' convenience for callers who only need a plain vector of p-values
#' corrected; call `fdr_correction(p, method = "BY")` instead for the
#' full result.
#'
#' @section Typical use:
#' Supply one family of raw p-values to [fdr_correction()] with
...
#' procedure used internally by [fdr_correction()]. Exported as a
#' standalone convenience for callers who only need a plain vector of
#' p-values corrected; call `fdr_correction(p, method = "BKY")`
...
#' @return A list with `q_value` (adjusted p-values, `NA` for cells
...
fdr_pvalue_histogram <- function(p, path = NULL) {
  ...
  plot_hist()
  if (!is.null(path)) .save_current_plot(path, plot_hist)
  invisible(NULL)
}
...
fdr_significance_maps <- function(result, path = NULL) {
  ...
  # This package's own chosen magenta (see .sptrends_brand, internal),
  ...
  plot_panels()
  if (!is.null(path)) {
    .save_current_plot(path, plot_panels, width = 1400, height = 900,
                       res = 130)
  }
  invisible(NULL)
}
...
fdr_comparison_barplot <- function(result, path = NULL) {
  ...
  panel <- function(sig, title) {
    ...
    bp <- graphics::barplot(vals, names.arg = labels,
                             col = c("#D9D9D9", .sptrends_brand$navy),
  ...
  plot_panels <- function() {
    ...
    on.exit(graphics::par(op), add = TRUE)
    panel(result$reject_raw,
          sprintf("Raw, uncorrected (p <= %.2f)", result$q))
    if (!is.null(result$reject_BH)) {
...
#' m0_hat / m` -- rather than a fixed `q`. Every rank at or before the
...
fdr_threshold_plot <- function(result, path = NULL) {
  ...
  panel <- function(thresh, k, title, ylim) {
    ...
    # appears. The light magenta (.sptrends_brand$cyan) marks the
    # threshold line itself, distinct from both navy and "firebrick"
    # (the cutoff), so all three stay visually separable.
    rejected <- if (is.na(k)) rep(FALSE, length(rng)) else rng <= k
    graphics::plot(rng, du$p_sorted, pch = 19,
         col = ifelse(rejected, .sptrends_brand$navy, "#D9D9D9"),
    ...
    graphics::legend("bottomright",
           ...
           col = c(.sptrends_brand$navy, "#D9D9D9", .sptrends_brand$cyan,
                   "firebrick"),
           pch = c(19, 19, NA, NA),
...
#' @param path Character or `NULL`. If supplied, a PNG is written there.
#' @param main Plot title. Defaults to the generic `"Binarised trend
#'   map"`; `workflow_tst()`/`workflow_rta()` results pass their own
#'   `"TST direction map"`/`"RTA direction map"` instead.
#' @return `NULL`, invisibly.
#' @family FDR correction functions
#' @references
...
fdr_direction_plot <- function(direction, path = NULL,
  ...
  plot_dir <- function() {
    .safe_categorical_plot(direction, values = c(-1, 0, 1),
                            ...
                            labels = c("Decrease", "Not significant",
                                      "Increase"),
                            main = main, reverse = TRUE)
  }
  plot_dir()
  if (!is.null(path)) .save_current_plot(path, plot_dir)
  invisible(NULL)
}
...
#'   (`c("raw", "BH", "BKY", "BY")` by default -- methods not present
#'   in `fdr_result` are skipped with a message, not an error).
#' @param path Character or `NULL`. If supplied, write the table to this
#'   CSV path.
#'
...
fdr_direction_summary <- function(trend, fdr_result, slope = NULL,
  ...
  skipped <- setdiff(c("raw", "BH", "BKY", "BY"), methods)
  if (length(skipped) > 0) {
