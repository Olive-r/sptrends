...
.summary_spatial_autocorrelation_local <- function(object, path = NULL, ...) {
  ...
  tab <- data.frame(
    ...
    alpha = object$alpha,
    significant_raw = sum(significant == 1, na.rm = TRUE)
  )
...
spatial_autocorrelation_summary <- function(result, path = NULL) {
  ...
  p_floor <- 1 / (result$nperm + 1)
  z_empirical <- if (null_sd > 0) {
  ...
  if (abs(result$p - p_floor) < 1e-12) {
    message(sprintf(
      "Note: p is at the mathematical floor for nperm=%d (1/(nperm+1)=%.4f).",
...
spatial_autocorrelation_null_plot <- function(result, path = NULL) {
  ...
  if (out_of_range) {
    ...
    txt <- sprintf("%s obs. %s = %.4f", arrow, stat_label, result$statistic)
    graphics::text(x = x_text, y = usr[4] * 0.85, labels = txt, col = "red",
         font = 2, adj = if (right_side) 1 else 0)
  } else {
    graphics::hist(result$null_dist, breaks = 20, col = "steelblue",
         border = "white",
         main = sprintf("Null distribution of %s (spatial permutation)",
    ...
    graphics::legend("topright",
           legend = sprintf("Observed %s", stat_label), col = "red", lwd = 2,
