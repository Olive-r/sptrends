...
# Save a plot as a PNG at an explicit path. Takes the plotting logic
# itself as a zero-argument function ('expr'), opening the PNG device
# BEFORE running it and closing it after -- never copies from an
# already-active device (grDevices::dev.copy()), which silently
# produces no file at all in headless/batch environments with no
# active graphics device (confirmed: this broke on CRAN's own Debian
# check machine, though not on Windows, where some device is
# apparently already available even non-interactively).
#' @noRd
.save_current_plot <- function(path, expr, width = 1000, height = 800,
                                res = 120) {
