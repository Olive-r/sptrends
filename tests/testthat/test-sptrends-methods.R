...
test_that("plot_detection_comparison() is internal again -- reverted from the 1.5.3 CRAN-driven export; reachable via plot() on a compare_detections() result", {
  exported <- getNamespaceExports("sptrends")
  expect_false("plot_detection_comparison" %in% exported)
})
...
test_that("print() on a Modified Mann-Kendall (MMK) trend_test result labels it correctly", {
  r <- sim_trend_stack(nrow = 4, ncol = 4, n_time = 12, seed = 1)$series
  resultado_mmk <- trend_test(r, method = "MMK", report = FALSE, verbose = FALSE)
  expect_output(print(resultado_mmk), "modified Mann-Kendall")
})

test_that("summary() on a global spatial_autocorrelation result computes the correct p-value floor with alternative = 'two.sided'", {
  r <- sim_trend_stack(nrow = 4, ncol = 4, n_time = 1, seed = 1)$series[[1]]
  resultado <- spatial_autocorrelation(r, scope = "global",
                                       alternative = "two.sided",
                                       nperm = 9, seed = 1,
                                       verbose = FALSE, report = FALSE)
  expect_error(summary(resultado), NA)
})
