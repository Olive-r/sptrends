...
test_that("workflow_tst()'s direction panel and slope panel agree on which slope source they use (both smoothed, not one raw)", {
  # Senal fuerte y uniforme para garantizar celdas significativas de
  # forma fiable, igual que en la bateria de pruebas de la sesion.
  ...
  slope_suavizado_esperado <- .mask_and_smooth_slope(
    result$theil_sen, reject, TRUE
  )

  # Lo que el panel "direction" deberia usar internamente ahora
  # (la pendiente ya suavizada), no la pendiente cruda -- confirma
  # que ambos paneles ("direction" y "slope") parten de la misma
  # fuente, arreglando la inconsistencia identificada en sesion.
  direccion_con_suavizado <- direction_map(
    result$trend, result$fdr, slope = slope_suavizado_esperado,
    method = "BKY", verbose = FALSE
  )
  direccion_sin_suavizar <- direction_map(
    result$trend, result$fdr, slope = result$theil_sen,
    method = "BKY", verbose = FALSE
  )

  vals_suave <- terra::values(direccion_con_suavizado, mat = FALSE)
  vals_crudo <- terra::values(direccion_sin_suavizar, mat = FALSE)

  # No exigimos que sean identicos (la suavizacion puede cambiar el
  # signo en celdas limitrofes, es precisamente el punto) -- pero SI
  # confirmamos que plot(result) no produce error con el mecanismo
  # nuevo, y que el objeto resultante sigue siendo una clasificacion
  # valida de -1/0/1/NA.
  expect_true(all(vals_suave %in% c(-1, 0, 1, NA)))
  expect_error(plot(result, which = "direction"), NA)
})
