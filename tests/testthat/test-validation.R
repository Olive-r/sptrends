...
test_that("compare_detections rejects a list-form detection with no 'significant' element, and a detection whose length does not match ground_truth", {
  expect_error(
    compare_detections(
      detections = list(m = list(no_es_significant = TRUE)),
      ground_truth = c(TRUE, FALSE, TRUE), verbose = FALSE
    ),
  ...
  expect_error(
    compare_detections(
      detections = list(m = c(TRUE, FALSE)),
      ground_truth = c(TRUE, FALSE, TRUE), verbose = FALSE
    ),
    "differ in length"
  )
