...
test_that("fdr_bh(), fdr_by() and fdr_bky() are internal again -- reverted from the 1.5.3 CRAN-driven export, since none had real standalone value beyond what fdr_correction() itself gives", {
  exported <- getNamespaceExports("sptrends")
  expect_false("fdr_bh" %in% exported)
  expect_false("fdr_bky" %in% exported)
  expect_false("fdr_by" %in% exported)
})

test_that("fdr_bh()/fdr_by()/fdr_bky() still work when called from within the package, and fdr_correction() still calls them correctly", {
  p <- c(0.001, 0.01, 0.02, 0.5, 0.8)
  expect_error(sptrends:::fdr_bh(p), NA)
  expect_error(sptrends:::fdr_by(p), NA)
  expect_error(sptrends:::fdr_bky(p), NA)
...
test_that("workflow_summary() is no longer part of the public API (method_citation(), classify_moran() remain internal; workflow_summary() itself was removed entirely, not just hidden)", {
  exported <- getNamespaceExports("sptrends")
...
test_that("prepare_cmk_neighbourhood() is part of the public API (exported for the repeated-call optimisation with precomputed_neighbourhood, per CRAN feedback on ':::' usage in examples)", {
  exported <- getNamespaceExports("sptrends")
  expect_true("prepare_cmk_neighbourhood" %in% exported)
})
