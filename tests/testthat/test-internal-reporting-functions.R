...
test_that("fdr_bh() and fdr_bky() are part of the public API (exported directly for callers who only need a plain vector of p-values corrected, per CRAN feedback on ':::' usage in examples)", {
  exported <- getNamespaceExports("sptrends")
  expect_true("fdr_bh" %in% exported)
  expect_true("fdr_bky" %in% exported)
  expect_true("fdr_by" %in% exported)
})

test_that("fdr_bh()/fdr_bky() still work when called from within the package, and fdr_correction() still calls them correctly", {
