...
test_that("sim_trend_stack validates ar1 as a finite scalar strictly inside (-1, 1) -- an external audit found that an out-of-bounds value silently produced NaN via sqrt(1 - ar1^2) instead of a clear error", {
  for (bad in list(-1, 1, -1.5, 1.5, NA_real_, Inf, c(0, 0.5), "high")) {
    expect_error(sim_trend_stack(ar1 = bad), "ar1")
  }
  expect_no_error(sim_trend_stack(nrow = 5, ncol = 5, n_time = 3,
                                   ar1 = 0, seed = 1))
  expect_no_error(sim_trend_stack(nrow = 5, ncol = 5, n_time = 3,
...
test_that("sim_trend_stack validates noise_sd as a finite non-negative scalar", {
  for (bad in list(-1, -0.01, NA_real_, Inf, c(0, 1), "loud")) {
    expect_error(sim_trend_stack(noise_sd = bad), "noise_sd")
  }
  expect_no_error(sim_trend_stack(nrow = 5, ncol = 5, n_time = 3,
                                   noise_sd = 0, seed = 1))
})

test_that("sim_trend_stack validates nrow, ncol and n_time as positive integers", {
  for (bad in list(0, -5, 2.5, NA_real_, Inf, c(5, 6), "wide")) {
    expect_error(sim_trend_stack(nrow = bad), "nrow")
    expect_error(sim_trend_stack(ncol = bad), "ncol")
    expect_error(sim_trend_stack(n_time = bad), "n_time")
  }
  # n_time = 1 is a real, documented use case (a single-layer raster
  # for spatial_autocorrelation() demonstrations), not an error --
  # only n_time <= 0 or non-integer should be rejected.
  expect_no_error(sim_trend_stack(nrow = 5, ncol = 5, n_time = 1, seed = 1))
})

test_that("sim_trend_stack validates trend_strength and break_magnitude as finite scalars (negative values are legitimate, only NA/NaN/Inf are rejected)", {
  for (bad in list(NA_real_, NaN, Inf, -Inf, c(0.1, 0.2), "strong")) {
    expect_error(sim_trend_stack(trend_strength = bad), "trend_strength")
    expect_error(sim_trend_stack(break_magnitude = bad), "break_magnitude")
  }
  expect_no_error(sim_trend_stack(nrow = 5, ncol = 5, n_time = 3,
                                   trend_strength = -0.2, seed = 1))
})

test_that("sim_trend_stack validates constant_block as a single logical value", {
  for (bad in list("TRUE", 1, NA, c(TRUE, FALSE))) {
    expect_error(sim_trend_stack(constant_block = bad), "constant_block")
  }
  expect_no_error(sim_trend_stack(nrow = 5, ncol = 5, n_time = 3,
                                   constant_block = FALSE, seed = 1))
})

test_that("sim_trend_stack validates seed as NULL or a finite scalar", {
  for (bad in list(NA_real_, Inf, "1", c(1, 2))) {
    expect_error(sim_trend_stack(seed = bad), "seed")
  }
  expect_no_error(sim_trend_stack(nrow = 5, ncol = 5, n_time = 3,
                                   seed = NULL))
})
