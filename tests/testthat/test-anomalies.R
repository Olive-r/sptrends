...
test_that("a shifted repeating cycle has zero anomalies and ordered climatology", {
  r <- .make_pattern_raster(c(20, 30, 10, 20, 30, 10), nrow = 1, ncol = 1)
  res <- compute_anomalies(r, cycle = 3, start_position = 2, verbose = FALSE)
  expect_equal(as.numeric(terra::values(res$anomalies)), rep(0, 6))
  expect_equal(as.numeric(terra::values(res$climatology)), c(10, 20, 30))
})

test_that("shifted partial cycles match independent means and standard deviations", {
  # Unequal group sizes and variances expose both mean and SD misalignment.
  values <- c(20, 30, 10, 24, 38, 12, 28, 46)
  r <- .make_pattern_raster(values, nrow = 1, ncol = 1)
  for (first in 1:3) {
    positions <- rep(c(first:3, if (first > 1) seq_len(first - 1)), length.out = 8)
    means <- vapply(1:3, function(j) mean(values[positions == j]), numeric(1))
    sds <- vapply(1:3, function(j) stats::sd(values[positions == j]), numeric(1))
    res <- compute_anomalies(r, cycle = 3, start_position = first,
                             standardise = TRUE, verbose = FALSE)
    expect_equal(as.numeric(terra::values(res$climatology)), means)
    expect_equal(as.numeric(terra::values(res$climatology_sd)), sds)
    expect_equal(as.numeric(terra::values(res$anomalies)),
                 (values - means[positions]) / sds[positions])
  }
})
