...
test_that("annual dates use 1 January across leap years with either range mode", {
  dir <- tempfile("sptrends_annual_"); dir.create(dir)
  on.exit(unlink(dir, recursive = TRUE))
  files <- .write_plain_files(dir, 3, prefix = "annual")
  expected <- as.Date(c("2019-01-01", "2020-01-01", "2021-01-01"))
  for (last in list(NULL, as.Date("2021-12-31"))) {
    r <- read_ordered_stack(files = files, cycle_type = "annual",
                            start = expected[1], end = last,
                            report = FALSE, verbose = FALSE)
    expect_equal(terra::time(r), expected)
  }
})
