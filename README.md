...
[![CRAN status](https://www.r-pkg.org/badges/version/sptrends)](https://CRAN.R-project.org/package=sptrends)
...
```r
install.packages("sptrends")
```

Or install the development version from GitHub:

```r
remotes::install_github("Olive-r/sptrends")
```
...
monthly layer is its centre. Annual layers use 1 January. Anomalies group
layers by position, not by their date metadata; the user supplies the
correct cycle and starting position. `files` plus `time` also supports an
explicit date for every layer. See the
[loading guide](https://olive-r.github.io/sptrends/articles/a-getting-started.html).
...
The 1.6.3 local check completed in under 10 minutes with 0 errors,
0 warnings and 0 notes. Coverage is 100% across every R source file with
`SPTRENDS_TEST_PARALLEL=true`. win-builder checks (R-devel, R-release,
R-oldrelease) had 0 errors and 0 warnings, with only the routine
submission/terminology note. See `cran-comments.md` for the current
validation status. The 19 parallel-execution tests remain optional to
keep routine checks within CRAN's time limit. Website building is a
separate step. Style, spelling and link integrity are checked
periodically with `lintr`, `goodpractice`, `spelling` and `urlchecker`.

Beyond the automated `tests/testthat/` suite, the package's Mann-Kendall
and Theil-Sen implementations have been cross-checked against five
independent CRAN packages (`Kendall`, `trend`, `modifiedmk`,
`robslopes`, `zyp`) across five test series (clear trend, no trend,
tied values, short, long), with no unexplained discrepancies. The
package has also undergone a comprehensive integral audit of code,
...
> Inference for Spatiotemporal Trends in Gridded Data. R package version
> 1.6.3, CRAN. <https://doi.org/10.32614/CRAN.package.sptrends>

Archived source code snapshots for each release are also available via
