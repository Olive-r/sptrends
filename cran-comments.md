...
* Local: `devtools::check(args = "--as-cran")` run directly against
  1.6.2: 0 errors, 0 warnings, 0 notes. Duration under 10 minutes
  (cloud-sync client paused; longer runs observed with it active
  are a local environment artefact, not a package issue).
...
* win-builder (R-devel, R-release, R-oldrelease), run against 1.6.1:
  ...
  No check-relevant change (test/example structure, dependencies)
  was made in 1.6.2, which only reverts four exports to internal.
...
Most recent local check, against 1.6.2: 0 errors, 0 warnings, 0 notes.
