...
- alpha:
  ...
  Numeric. Significance threshold used for the significance and
...
- panels:
  ...
  Character vector: which of the 4 available maps to draw, any subset of
  `c("statistic", "pvalue", "significance", "direction")` (default: all
  four, in that order). `"statistic"`: the raw `Sm`/`S` trend statistic
  (diverging). `"pvalue"`: the uncorrected p-value (sequential).
...
Invisibly, a list with `significant` and `classes` (both `SpatRaster`,
`NULL` if `"significance"`/`"direction"` respectively was not in
`panels`).
