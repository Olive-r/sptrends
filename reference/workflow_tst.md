...
- prewhiten_args:
  ...
  **Known limitation with irregular time spacing and `TFPW_Y`:** this
  ...
  observation times are irregularly spaced (not one evenly-spaced step
  per layer) and you need a genuine time axis for
  [`trend_test()`](https://olive-r.github.io/sptrends/reference/trend_test.md)/[`slope_estimator()`](https://olive-r.github.io/sptrends/reference/slope_estimator.md)
  rather than the default `1:n` layer index, you must supply the correct
  `t` yourself in `prewhiten_args`, `cmk_args` and `theil_sen_args`
  separately, and keep them consistent by hand. `TFPW_Y` specifically
  drops the first observation (see
  [`prewhiten()`](https://olive-r.github.io/sptrends/reference/prewhiten.md)'s
  own "Value" section), so the `t` you pass to
  `cmk_args`/`theil_sen_args` after using `TFPW_Y` must itself have its
  own first element dropped to stay aligned with the one-layer-shorter
  prewhitened series – this workflow does not do that adjustment for
  you. Not an issue for `TFPW_WS` (default), which keeps every layer.
...
- cmk_args:
  ...
  **(Trend detection)** A named list of extra arguments forwarded to
  ...
  published workflow. See `prewhiten_args` above for a known limitation
  with irregular time spacing and `TFPW_Y` if you pass `t` here.
...
- theil_sen_args:
  ...
  computational cost" above for why. See `prewhiten_args` above for a
  known limitation with irregular time spacing and `TFPW_Y` if you pass
  `t` here.
...
- trend:
  ...
  own output (a `SpatRaster` with the trend statistics themselves) – not
  the full
  [`trend_test()`](https://olive-r.github.io/sptrends/reference/trend_test.md)
  return value, which also includes `neighbourhood`/`window_size`.
...
- theil_sen:
  ...
  The `$slope` field of
  [`slope_estimator()`](https://olive-r.github.io/sptrends/reference/slope_estimator.md)'s
  own output (a `SpatRaster`), or `NULL` if `theil_sen = FALSE`. Not the
  full
  [`slope_estimator()`](https://olive-r.github.io/sptrends/reference/slope_estimator.md)
  return value. Not smoothed by default (see the `theil_sen_args` note
  above) – check `theil_sen_smoothed` if you need to know whether it was
  (e.g. because you set it yourself).
